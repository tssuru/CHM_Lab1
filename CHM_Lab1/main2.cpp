...
int main() {
    ...
    std::cout << "Точність (epsilon): " << epsilon << std::endl;
    std::cout << "Початкове наближення (x0): " << x0 << std::endl << std::endl;
    ...
    if (k == max_iterations) {
        std::cout << "Метод не збігається після " << max_iterations << " ітерацій." << std::endl;
    } else {
        std::cout << std::endl << "Знайдений корінь: x = " << x_prev << std::endl;
        std::cout << "Кількість ітерацій: " << k << std::endl;
    }
